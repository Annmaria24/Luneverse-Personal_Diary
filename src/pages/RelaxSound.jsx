...
function RelaxSound({ embedded = false }) {
  ...
  useEffect(() => {
    ...
    sounds.forEach((sound) => {
      if (pendingPlay.current[sound.name]) {
        try { createYTPlayer(sound); } catch {} // eslint-disable-line no-empty
      }
    });
  ...
  useEffect(() => {
    return () => {
      ...
      // eslint-disable-next-line react-hooks/exhaustive-deps
      const ytPlayersCopy = ytPlayers.current;
      sounds.forEach(sound => {
        const audio = audioRefsCopy[sound.name];
        if (audio) {
          audio.pause();
          audio.currentTime = 0;
        }
        ...
        const ytPlayer = ytPlayersCopy[sound.name];
        if (ytPlayer) {
          try {
            ytPlayer.pauseVideo();
