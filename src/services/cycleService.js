...
import { calculateCyclePhase } from "./cyclePhaseService";
  ...
  try {
    ...
    if (periodEntries.length > 0) {
      ...
      if (allPeriodStarts.length > 0) {
        const lastStart = allPeriodStarts[0].date;
        stats.lastPeriodStart = lastStart;
        stats.lastPeriodDate = lastStart;

        // Use the unified cyclePhaseService for calculations
        const phaseCalculations = calculateCyclePhase(
          lastStart,
          stats.averageCycleLength,
          stats.averagePeriodLength
        );

        stats.currentCycleDay = phaseCalculations.cycleDay;
        stats.currentPhase = phaseCalculations.cyclePhase;
        stats.nextPredictedPeriod = phaseCalculations.predictedNextPeriodDate.toISOString().split('T')[0];
      }
